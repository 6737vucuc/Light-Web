...
export async function GET(
  ...
  try {
    ...
    const messages = await sql`
      ...
      FROM messages
      WHERE (
        (sender_id = ${decoded.userId} AND receiver_id = ${otherUserId} AND deleted_by_sender = false)
        OR 
        (sender_id = ${otherUserId} AND receiver_id = ${decoded.userId} AND deleted_by_receiver = false)
      )
